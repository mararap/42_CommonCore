...
int	is_safe(int x, int y)
{
	...
	while (x_temp < x)					// for every column-index until we reach the current
	{
		...
		if (value == y)					// if there is a queen in the same line
			return (0);
...
void	find_solutions(int x)		// function called with 0
{				    				// therefor first column-idx = 0
	int y = 0; 
    ...
    {	
		...
		{							// aka position at row-idx X col-idx is save,
			...
			find_solutions(x + 1);	// call solve-function recursivly for x + 1
...
int	main(int ac, char **av)
{
	...
	if (!g_solution)
		return (1);
	find_solutions(0);						// call function
